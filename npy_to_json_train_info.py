...
#    d_sample = 3e8/(2*neff*sample_rate)   # sampling distance [m]
    ...
    event = json_dict.get("info").get("event")
    train_id = json_dict.get("info").get("train-id")
    train_id_confidence = json_dict.get("info").get("train-id-confidence")
    train_id_confidence_str = train_id_confidence.replace(".", "_")
    logger.info(f"Detected event: {event}")
    
    save_filename = '%s/%s_%s_id_%s_conf_%s'%(json_image_day_folder, str_timestamp, event, train_id, train_id_confidence_str)
    ...
    year, month, day = train_datetime.year, train_datetime.month, train_datetime.day
    report_filename = "report_files_%s_%s_%s.json"%(year, month, day)
...
#                logger.info("report_path saved! 01: %s"%report_path)
        ...
        try:
            with open(report_path, 'w', encoding='utf-8') as file:
                json.dump(report_json, file)
#                logger.info("report_path saved! 02!!: %s"%report_path)
...
def main(npy_data_folder, json_main_save_folder, json_main_image_folder, now, report_folder):
    ...
    if not os.path.exists(npy_day_folder): 
        ...
        str_timestamp_list = [filename.split(separator_str)[0] for filename in valid_filename_list if separator_str in filename]
        ...
        for str_timestamp in str_timestamp_list: 
            ...
            if str_timestamp not in processed_files_list: 
                ...
                if os.path.exists(last_buffer_file): 
                    ...
                    logger.info('Error: buffer not completed yet, mising file: %s_2.npy, try again later' %(str_timestamp))
...
if __name__ == '__main__': 
    ...
    report_folder = '/mnt/Data/Preprocessed_phase/report_files'
...
#        (year, month, day) = (2024,4,8)
        ...
        try:
            ...
            main(npy_data_folder, json_main_save_folder,json_main_image_folder, now, report_folder)
            logger.info("-----------------------------------------------")
